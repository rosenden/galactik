...
const ContrastMatrixViewer: React.FC<{ data: any; title: string; showLevel?: string }> = ({
  ...
  const stats = useMemo(() => {
    const items = data.validCombinations
      ? [...(data.validCombinations.AAA || []), ...(data.validCombinations.AA || [])]
      : data.forbiddenCombinations || [];
    const AAA = items.filter((c: ContrastData) => c.level === 'AAA').length;
    const AA = items.filter((c: ContrastData) => c.level === 'AA').length;
    return { total: items.length, AAA, AA };
  }, [data]);
    ...
    <div style={{ padding: '2rem', fontFamily: 'system-ui' }}>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ margin: 0, color: '#666', fontSize: '0.9rem' }}>
          {data.statistics && (
            <>
              {`Total combinations: ${data.statistics.total.toLocaleString()} | Valid: ${data.statistics.valid?.toLocaleString() || data.statistics.validCombinations} | AAA: ${stats.AAA.toLocaleString()} | AA: ${stats.AA.toLocaleString()}`}
            </>
          )}
        </p>
      </div>
