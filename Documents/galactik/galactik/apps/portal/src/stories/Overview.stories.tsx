...
const quickSteps: QuickStep[] = [
  {
    ...
    hint: 'Le workspace couvre packages/* et apps/*.'
  },
  {
    title: '2. Lancer un Storybook ciblé',
    description: 'pnpm storybook:<framework>',
    hint: 'angular · react · vue · wc · portal'
  },
  {
    title: '3. Build multi-framework',
    description: 'pnpm build:storybooks',
    hint: 'Génère dist/storybooks/* + portail agrégé.'
  }
];

const commands: Command[] = [
  { label: 'pnpm storybook:react', description: 'React UI (port 6006) – builder Vite.' },
  { label: 'pnpm storybook:angular', description: 'Angular UI (port 6008) via @storybook/angular.' },
  { label: 'pnpm storybook:vue', description: 'Vue 3 UI (port 6007) avec plugin Vite.' },
  { label: 'pnpm storybook:wc', description: 'Web Components (port 6009) après build Stencil.' },
  { label: 'pnpm --filter portal storybook', description: 'Portail agrégateur (port 6010).' },
  { label: 'pnpm preview:storybooks', description: 'Build + serveur statique (port 5000).' }
];

const resources: Resource[] = [
  {
    title: 'Tokens & styles',
    description: 'Fondations typographiques + couleurs partagées.',
    href: '/styles/tokens.css'
  },
  {
    title: 'Scripts Storybook',
    description: 'build-storybooks.mjs & serve-storybooks.mjs',
    href: '/scripts'
  },
  {
    title: 'Chromatic',
    description: 'Capture visuelle via pnpm chromatic (token requis).',
...
const frameworkCards = STORYBOOK_REFS.filter((ref) => ref.id !== 'portal').map((ref) => {
  ...
  const localUrl = `http://localhost:${ref.port}${ref.path}/`;
  return {
    id: ref.id,
    label: ref.label,
      ...
      >
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: 'var(--font-size-lg)', maxWidth: 720, lineHeight: 1.6 }}>
            Ce hub rassemble tous les Storybooks mono-framework, les commandes PNPM clés et les
            liens internes pour accélérer vos intégrations produit.
          </p>
        </section>
        ...
        <section>
          <h2 style={{ marginBottom: 16, fontSize: 22, color: 'var(--color-font-primary-base)' }}>Ressources Galactik</h2>
          <div
            style={{
              display: 'grid',
              gap: 16,
